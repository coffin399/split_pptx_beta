...
**Q. Web版とデスクトップ版の違いは？**  
A. Web版はブラウザから利用できインストール不要です。デスクトップ版はオフラインで利用でき、より詳細な設定が可能です。

**Q. Render.com でのデプロイに必要なものは？**  
A. GitHubリポジトリとRender.comアカウントのみ。Dockerfileとrender.yamlが自動的にデプロイを設定します。

**Q. メモリ使用量が多いですが？**  
A. 400MBを超えると自動クリーンアップが作動します。完了したタスクは1時間後に自動削除されます。

**Q. 日本語フォントが表示されない？**  
A. Dockerfileで日本語フォントをインストールしています。ローカルの場合はOSの日本語フォントを確認してください。
...
### Web版
- PPTXファイルが .pptx 形式であることを確認
- ファイルサイズが大きすぎないか（最大50MB推奨）
